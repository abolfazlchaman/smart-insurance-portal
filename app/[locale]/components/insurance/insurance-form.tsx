...
import { Heart, Home, Car, HelpCircle, Info, AlertTriangle, Check } from 'lucide-react';
      ...
      const hasRequiredFields = requiredFields.every((field) => {
        ...
        if (field.type === 'group') {
          return field.fields?.every((nestedField) => {
            if (!nestedField.required || !isFieldVisible(nestedField)) return true;
            return formData[nestedField.id] != null && formData[nestedField.id] !== '';
          });
        }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = {
        ...
        'Full Name': `${formData.first_name || ''} ${formData.last_name || ''}`.trim() || 'N/A',
        'Age': age,
                ...
                {field.options.map((option) => (
                  <label
                    ...
                    <input
                      ...
                      name={field.id}
                      value={option}
                      required={field.required && field.id !== 'has_safety_measures'}
                      className='radio'
                      onChange={(e) => handleInputChange(field.id, e.target.value)}
                    />
                  </label>
                  ...
                  <div
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (value.length > 4 || parseInt(value) < 1990 || parseInt(value) > 2025) {
                          e.target.value = '1990';
